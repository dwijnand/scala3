...
object TypeOps:
  ...
  final def asSeenFrom2(tp: Type, pre: Type, cls: Symbol)(using Context): Type = {
    pre match {
      case pre: QualSkolemType =>
        // When a selection has an unstable qualifier, the qualifier type gets
        // wrapped in a `QualSkolemType` so that it may appear soundly as the
    ...
    new AsSeenFromMap2(pre, cls).apply(tp)
  }

  /** The TypeMap handling the asSeenFrom */
  class AsSeenFromMap2(pre: Type, cls: Symbol)(using Context) extends ApproximatingTypeMap {
    ...
    def apply(tp: Type): Type = {
      ...
      def toPrefix(pre: Type, cls: Symbol, thiscls: ClassSymbol): Type = trace/*.force*/(i"toPrefix($pre, $cls, $thiscls)", show = true) {
      ...
      trace/*.force*/(i"asSeen $tp from ($pre, $cls)  ${tp.getClass.getSimpleName}", show = true)(tp match
        case tp: NamedType =>
          val sym = tp.symbol
          if sym.isStatic && !sym.maybeOwner.seesOpaques || (tp.prefix eq NoPrefix) then tp
          else derivedSelect(tp, atVariance(variance max 0)(this(tp.prefix)))
        case tp: LambdaType => mapOverLambda(tp) // special cased common case
        case tp: ThisType   => toPrefix(pre, cls, tp.cls)
        case _: BoundType   => tp
        case _              => mapOver(tp))
    }

    override def mapOver(tp: Type): Type = trace/*.force*/(i"asSeen $tp from ($pre, $cls)  ${tp.getClass.getSimpleName}", show = true)(super.mapOver(tp))

    override def reapply(tp: Type): Type = tp // derived infos have already been subjected to asSeenFrom, hence to need to apply the map again.
